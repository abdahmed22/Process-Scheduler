...
struct Node* createNode(int id , int arrivaltime, int remainingtime , int priority , int PID)
{
...
void Enqueue(struct Queue** Q,struct Process ProcessObj,enum SchedulerALgo i)
{
    ...
    struct Node* CreatedNode = createNode(ProcessObj.ID,ProcessObj.ArrivalTime,ProcessObj.RemainingTime,ProcessObj.Priority,ProcessObj.ParentID);
